...
- **Framework** (e.g., Next.js, FastAPI, Django, Express, SvelteKit, Astro)
- **Styling** (e.g., Tailwind CSS 4, CSS Modules, vanilla CSS)
- **UI Components** (e.g., shadcn/ui, custom components, none)
- **Database** (e.g., PostgreSQL self-hosted, SQLite, MongoDB, none -- PostgreSQL is the default for all web apps)
- **ORM / Data Layer** (e.g., Prisma, Drizzle, SQLAlchemy, none)
- **Authentication** (e.g., Auth.js v5, Clerk, custom, none)
- **CMS** (e.g., Sanity, none -- only if content-heavy)
- **Testing** (e.g., Vitest + happy-dom, Pytest, Playwright)
- **Package manager** (e.g., pnpm, pip/uv, cargo)
- **Linting & formatting** (e.g., ESLint + Prettier, Ruff, Clippy)
- **Version control** (Git -- always)
- **CI/CD** (if applicable)
- **Additional tools** (anything project-specific: email service, payment processor, AI SDK, analytics, etc.)

**Standard stack paths:**

| Path | Stack | When to Use |
| --- | --- | --- |
| **Web App (default)** | Next.js + TypeScript + Tailwind 4 + shadcn/ui + pnpm | Websites, web apps, full-stack with UI |
| **API / Backend** | Python + FastAPI + SQLAlchemy + asyncpg + PostgreSQL | Pure APIs, data pipelines, AI/ML backends |
| **Static Site** | HTML/CSS/JS (no framework) | Reference sites, documentation, zero-dep |
| **Content Platform** | Next.js + TypeScript + Sanity CMS + Tailwind 4 | Blog, knowledge base, editorial content |
...
TECH STACK (LOCKED after approval)
  ...
  UI Components:  [chosen component library or "custom"]
  Database:       [chosen database or "none"]
  ORM:            [chosen ORM or "none"]
  Auth:           [chosen auth or "none"]
  CMS:            [chosen CMS or "none"]
...
3. **Configure linting & formatting** -- Strict rules aligned with Code Quality Standards (Part II, Section B)
4. **Configure language settings** -- Strict mode, appropriate compiler options (zero `any` in TypeScript, strict mypy in Python)
5. **Configure environment variable validation** -- Set up schema-based validation (e.g., Zod for TS, pydantic-settings for Python) so the app fails fast at startup if required env vars are missing. Import the validated env object instead of reading raw env vars directly
...
7. **Create governance files:**
   ...
   - `HANDOFF.md` -- Session continuity tracker. Contains: what's done, what's in-progress, what's next, blockers, and decisions made. Updated at the end of every session
   - `AUDIT.md` -- Standalone audit file. Contains: Health Dashboard scores, Issues Tracker (open/fixed/deferred), Tech Debt Register, Security Posture checklist, Accessibility Status, and Audit History log. This is the living record of project quality
   - `ProjectHealth.md` -- High-level project health summary. Contains: overall grade, dimension scores, codebase metrics, open issue count, and a production readiness checklist
...
12. **Commit standards to AI memory** -- Write the Eight Pillars, conflict resolution hierarchy, prohibited actions, session protocol, and key standards into the AI's persistent project memory files. At minimum, the memory should contain:
...
After the codebase is scaffolded (Step 5), set up the foundational infrastructure. These guides are reusable across projects and must be completed before feature development begins.

> **Note:** Steps 6a-6d run in local dev. VPS deployment happens later when the site is ready to show the client. Complete 6a, 6b, and 6d locally first.
...
#   node_modules/, .next/, /build, /out, __pycache__/, .venv/
#   /coverage, /test-results, /playwright-report
#   *.pem, .DS_Store, Thumbs.db, tmp/
...
> **Multi-account note:** If using multiple GitHub accounts, see `~/.claude/docs/ssh-setup.md` for the SSH alias pattern. Use SSH remotes (`git@github.com-ALIAS:`) instead of HTTPS.
...
Create `.github/workflows/ci.yml` to run quality gates on every push. Adapt commands to your stack:
...
PostgreSQL is the default database for all web applications. SQLite is acceptable for simple projects without concurrent writes.

**Prerequisites:**

- PostgreSQL installed (v17 preferred, v15+ minimum)
- `psql` accessible from terminal
- ORM configured in the project (Prisma, Drizzle, or SQLAlchemy)
...
| **Strong password** | 20+ characters, alphanumeric (avoid special chars in connection strings) |
...
> **Gotcha:** Drizzle-kit doesn't auto-load `.env.local`. Prefix commands: `DATABASE_URL=$(grep DATABASE_URL .env.local | cut -d= -f2-) pnpm drizzle-kit push`
...
pnpm db:migrate     # Production migrations (when schema is stable)
```

For Drizzle:

```bash
pnpm drizzle-kit push    # Push schema
...
- [ ] Schema push/migration succeeds
- [ ] Seed succeeds and is idempotent
- [ ] Application connects and queries work

---

### Step 6c: VPS & NGINX Setup (Deferred)

> This step happens when deploying to VPS -- not during local development.

Full VPS setup instructions (NGINX, SSL, PM2, firewall, fail2ban, service users) are maintained in the global knowledge base:

- **`~/.claude/docs/vps-infrastructure.md`** -- Server details, port assignments, deploy flow, PM2 commands, firewall, security
- **`~/.claude/docs/ssh-setup.md`** -- Multi-account SSH, GitHub deploy keys, VPS SSH config
- **`~/.claude/docs/reusable-patterns.md`** -- Deploy script pattern, service user SSH setup

**Key principles (apply at deployment time):**

- Dedicated nologin service user per project
- NGINX reverse proxy (never expose app ports directly)
- SSL via Let's Encrypt with auto-renewal
- PM2 process manager with ecosystem.config.cjs
- UFW firewall: allow only SSH port, 80, 443
- Fail2ban for intrusion prevention
- All commands run as the service user, never as root
...
Server boots
  ...
  -> systemd starts PM2 via startup script (per service user)
    -> PM2 starts the app on its assigned port
  -> NGINX proxies port 443 -> app port
...
SEO is foundational infrastructure, not an afterthought. Set up the skeleton at project start so every page ships with proper metadata from day one.
...
**Sitemap, Robots, AI Engine files:**

- `sitemap.ts` -- Dynamic sitemap that queries DB for dynamic pages. Priority: homepage 1.0, key pages 0.9, secondary 0.7-0.8
- `robots.ts` -- Allow `/`, disallow `/api/`, `/login`, `/dashboard/`, any private routes. Include sitemap URL
- `llms.txt` -- Concise summary (under 50 lines): business name, location, hours, services, highlights, contact
- `llms-full.txt` -- Comprehensive: full service descriptions, team bios, FAQ with answers, areas served

**OG image requirements:**

- Minimum 1200x630px, WebP format preferred
- Include business branding, descriptive alt text
...
- [ ] JSON-LD on service/product pages
...
INFRASTRUCTURE
  ...
  Database:   Created + user + schema pushed + seed run
  SEO:        Metadata, sitemap, robots, JSON-LD, llms.txt ready
  VPS/NGINX:  Deferred to deployment phase
...
> **Cross-reference:** Technical gotchas and stack-specific patterns are maintained in `~/.claude/docs/technical-patterns.md`. This section covers universal standards that apply regardless of stack.
...
> **Gotcha:** After enabling HTTPS, update `SITE_URL` env var to `https://` or Origin header validation will fail.
...
| **.env permissions** | chmod 600 on VPS -- only readable by owning service user |

**Environment Variable Validation:**

- Define a schema (Zod for TS, pydantic-settings for Python) for all required environment variables
...
| **Cross-project audits** | Track results in `~/.claude/docs/global-audit.md` |

### A.11 Infrastructure Security (applies at deployment time)

| Layer | Standard |
| --- | --- |
| **SSH** | Key-based auth (Ed25519), password auth disabled, non-standard port |
...
| **Reverse proxy** | Version hidden, HTTP-to-HTTPS redirect, app ports closed in UFW |
| **Service isolation** | Dedicated nologin user per project, all commands run as service user |

> **Full infrastructure details:** `~/.claude/docs/vps-infrastructure.md`
...
| `AUDIT.md` | Health dashboard, issues tracker, tech debt, audit history | When issues found/fixed |
| `ProjectHealth.md` | Overall grade, dimension scores, production readiness | When health changes |
...
| Functions / Variables | camelCase (TS) or snake_case (Python) | `formatDate`, `format_date` |
...
| Python | `# === SECTION ===` with docstrings |
...
| **Bullet card format** | For long reference lists (tables generate lint warnings at scale) |
...
- **Never run git/build/PM2 commands as root on VPS** -- always use the service user
...
| Rule | Standard |
| --- | --- |
| **No em-dashes in public content** | Use hyphens (-) or double dashes (--) instead |
...
| 2026-03-09 | Step 6 (NEW) | Added Infrastructure Foundation: 6a GitHub, 6b PostgreSQL, 6c NGINX, 6d SEO -- full setup guides | Missing practical how-to for foundational infrastructure |
...
| 2026-04-02 | Full document | **Version 4.0 rewrite.** Trimmed VPS/NGINX/SSH infrastructure details into references to global knowledge base docs. Added Python/FastAPI stack path. Updated to Next.js 16, Tailwind 4, Zod v4, Node 22. Added ORM/CMS/UI Components to stack config. Updated Co-Authored-By format. Added service user isolation rule to prohibited actions. Added Drizzle and SQLAlchemy ORM options. Consolidated Step 6c to reference `vps-infrastructure.md` instead of duplicating 200 lines of NGINX config. Added cross-project dependency audit reference to `global-audit.md`. Python naming conventions added to B.4. Markdown bullet card format note added to B.7. | Portfolio Quality Initiative -- full review and alignment with current 11-project knowledge base |
